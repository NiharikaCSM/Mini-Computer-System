...
void fetch(int proc_id) {
    int pc = PC[proc_id];
    opcode_a[proc_id] = (unsigned char)Instruction[proc_id][pc];
    dest_a[proc_id]   = (unsigned char)Instruction[proc_id][pc + 1];
    src1_a[proc_id]   = (unsigned char)Instruction[proc_id][pc + 2];
    src2_a[proc_id]   = (unsigned char)Instruction[proc_id][pc + 3];
    PC[proc_id] += 4;
}
...
void execute(int proc_id) {
    ...
    switch (opcode) {
        ...
        case OP_MEMREAD_VAR: //x_dest = [x_src2]  (address held in register src1)
            Register[proc_id][dest] = Data[proc_id][Register[proc_id][src2]];
            break;

        case OP_MEMREAD_CONST: //x_dest = [const]  (src2 IS the literal address)
            Register[proc_id][dest] = Data[proc_id][src2];
            break;

        case OP_MEMWRITE_VAR: //[x_dest] = x_src2 (address held in register dest)
            Data[proc_id][Register[proc_id][dest]] = (char)Register[proc_id][src2];
            break;

        case OP_MEMWRITE_CONST: //[const] = x_src2 (dest IS the literal address)
            Data[proc_id][dest] = (char)Register[proc_id][src2];
            break;
        ...
        case OP_VECTOR_READ_VAR: { //read from vector and store in [x]
            ...
            for (int i = 0; i < 8; i++, addr += 4) {
                Data[proc_id][addr] = (char)VRegister[proc_id][src2][i];
            }
            break;
        }
        ...
        case OP_VECTOR_WRITE_VAR: { //write into vector from [x]
            ...
            for (int i = 0; i < 8; i++, addr += 4) {
                VRegister[proc_id][dest][i] = Data[proc_id][addr];
            }
            break;
        }

        case OP_VECTOR_READ_CONST: //read from vector and store in [const]
            for (int i = 0, d = dest; i < 8; i++, d = d + 4) {
                Data[proc_id][d] = (char)VRegister[proc_id][src2][i];
            }
            break;

        case OP_VECTOR_WRITE_CONST: //write into vector from [const]
            for (int i = 0, tmp = src2; i < 8; i++, tmp = tmp + 4) {
                VRegister[proc_id][dest][i] = Data[proc_id][tmp];
            }
            break;
