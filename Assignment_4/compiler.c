...
int compile(const char *inputFile, const char *outputFile) {
    ...
    for (int i = 0; i < lineCount; i++) {
        ...
        /* Lab 4: Print <variable>, e.g. "print x1".
           Dest and operand1 are always 0; the register is operand2. */
        if (!matched && sscanf(programLine, " print x%d", &src2) == 1) {
            opcode = OP_PRINT;
            dest = 0;
            src1 = 0;
            matched = 1;
        ...
        // Vector Arithmetic: second operand is a variable
        if (!matched && sscanf(programLine, " v%d = v%d %c v%d ", &dest, &src1, &op, &src2) == 4) {
            if      (op == '+') opcode = OP_VECTOR_ADD_VAR_VEC;
            else if (op == '-') opcode = OP_VECTOR_SUB_VAR_VEC ;
            else if (op == '*') opcode = OP_VECTOR_MUL_VAR_VEC ;
            matched = 1;
        }

        // Vector Arithmetic: second operand is a constant
        if (!matched && sscanf(programLine, " v%d = v%d %c %d ", &dest, &src1, &op, &src2) == 4) {
            if      (op == '+') opcode = OP_VECTOR_ADD_CONST;
            else if (op == '-') opcode = OP_VECTOR_SUB_CONST ;
            else if (op == '*') opcode = OP_VECTOR_MUL_CONST ;
            matched = 1;
        }

        // Vector Arithmetic: second operand is a register
        if (!matched && sscanf(programLine, " v%d = v%d %c x%d ", &dest, &src1, &op, &src2) == 4) {
            if      (op == '+') opcode = OP_VECTOR_ADD_VAR_REG;
            else if (op == '-') opcode = OP_VECTOR_SUB_VAR_REG ;
            else if (op == '*') opcode = OP_VECTOR_MUL_VAR_REG ;
            matched = 1;
        }

        //Writing into vector from memory, operand is a variable
        if (!matched && sscanf(programLine, " v%d = [x%d] ", &dest, &src2) == 2){
            opcode = OP_VECTOR_WRITE_VAR;
            src1 = 0;
            matched = 1;
        }

        //Writing into vector from memory, operand is constant
        if (!matched && sscanf(programLine, " v%d = [%d] ", &dest, &src2) == 2){
            opcode = OP_VECTOR_WRITE_CONST;
            src1 = 0;
            matched = 1;
        }

        //Reading from vector and writing into memory, operand is variable
        if (!matched && sscanf(programLine, " [x%d] = v%d ", &dest, &src2) == 2){
            opcode = OP_VECTOR_READ_VAR;
            src1 = 0;
            matched = 1;
        }

        //Reading from vector and writing into memory, operand is constant
        if (!matched && sscanf(programLine, " [%d] = v%d ", &dest, &src2) == 2){
            opcode = OP_VECTOR_READ_CONST;
            src1 = 0;
            matched = 1;
        }
        ...
        //memory read, address is a constant
