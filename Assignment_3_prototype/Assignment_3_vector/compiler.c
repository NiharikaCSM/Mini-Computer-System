...
void compile(const char *inputFile, const char *outputFile) {
    ...
    for (int i = 0; i < lineCount; i++) {
        ...
        // Vector Extract: x1 = v2[0]
        if (!matched && sscanf(programLine, " x%d = v%d[%d] ", &dest, &src1, &src2) == 3) {
            opcode = OP_VEXTRACT;
            matched = 1;
        }

        // Vector Arithmetic: v1 = v2 + v3, v1 = v2 - v3, v1 = v2 * v3
        if (!matched && sscanf(programLine, " v%d = v%d %c v%d ", &dest, &src1, &op, &src2) == 4) {
            if      (op == '+') opcode = OP_VADD_VAR;
            else if (op == '-') opcode = OP_VSUB_VAR;
            else if (op == '*') opcode = OP_VMUL_VAR;
            matched = 1;
        }

        // Vector Constant Load: v1 = 10 (broadcasts 10 across all 4 lanes)
        if (!matched && sscanf(programLine, " v%d = %d ", &dest, &src1) == 2) {
            opcode = OP_VLOAD_CONST;
            src2 = 0;
            matched = 1;
        }
