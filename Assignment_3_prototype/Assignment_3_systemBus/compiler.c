...
void compile(const char *inputFile, const char *outputFile) {
    ...
    /* ---------- Pass 1: clean each line, collect label -> instruction index ---------- */
    int instrIndex = 0;
    for (int i = 0; i < lineCount; i++) {
        cleanUpLine(lines[i]);          // strip comment + lowercase FIRST
        if (isBlankLine(lines[i])) continue;   // ...THEN check for blank
        ...
        if (parseLabelLine(lines[i], labelName)) {
            ...
            continue; //labels do not occupy an instruction slot
        }
        instrIndex++;
    }
    ...
    /* ---------- Pass 2: parse each instruction and emit its 4 bytes ---------- */
    instrIndex = 0;
    ...
    for (int i = 0; i < lineCount; i++) {
        ...
        //legacy read: address is a constant -> dest=register, src1=address
        ...
        //legacy write: address is a constant -> dest=address, src1=register
        ...
        if (!matched && sscanf(programLine, " [%d] = x%d", &dest, &src1) == 2) {
            opcode = OP_MEMWRITE_CONST;
            src2 = 0;
            matched = 1;
        }
    ...
    //Writing halt instruction at eof (once, after the loop is done)
