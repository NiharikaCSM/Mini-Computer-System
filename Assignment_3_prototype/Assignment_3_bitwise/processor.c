...
static void updateFlagsBitwise(int result) {
    Z = (result == 0) ? 1 : 0;
    N = (result < 0) ? 1 : 0;
    C = 0; // Bitwise operations clear carry and overflow flags
    V = 0;
}
...
void execute(void) {
    switch (opcode) {
        ...
        case OP_DATAMOVE_VAR: //x_dest = x_src1 
            ...
            break;
        case OP_AND_VAR: {
            int result = Register[src1] & Register[src2];
            Register[dest] = result;
            updateFlagsBitwise(result);
            break;
        }
        case OP_AND_CONST: {
            int result = Register[src1] & src2;
            Register[dest] = result;
            updateFlagsBitwise(result);
            break;
        }
        ...
        case OP_RSHIFT_VAR: {
            // Unsigned cast performs logical right shift (filling with zeros)
            int result = (int)((unsigned int)Register[src1] >> Register[src2]);
            Register[dest] = result;
            updateFlagsBitwise(result);
            break;
        }
        case OP_RSHIFT_CONST: {
            int result = (int)((unsigned int)Register[src1] >> src2);
            Register[dest] = result;
            updateFlagsBitwise(result);
            break;
        }
